...
class AssetGroup(
    ...
    def materialize(
        ...
        Only supported by AssetGroups that use the default, in-process executor.
        ...
        if self.executor_def and self.executor_def is not in_process_executor:
            raise DagsterUnmetExecutorRequirementsError(
                "'materialize' can only be invoked on AssetGroups that use the in-process '"
                f"executor, but the AssetGroup had executor '{self.executor_def.name}'"
            )
